...
int ComportamientoJugador::Minimos(const stateN3 &st){
	...
	devolver = min(bateriaColab.at(0).at(st.colaborador.f).at(st.colaborador.c), devolver);
	return(devolver);
}
...
bool ComportamientoJugador::a_star(const stateN3 &inicio, const ubicacion &final){
	...
	while (!frontier.empty() and !SolutionFound){
		...
		cout << "nuevo:" << endl;
		cout << current_node.st.heuristica << endl; 
		cout << current_node.st.jugador.f << endl;
		cout << current_node.st.jugador.c << endl; 
		cout << current_node.st.colaborador.f << endl;
		cout << current_node.st.colaborador.c << endl;
